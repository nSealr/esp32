//! BIP-39 English wordlist (2048 words), sorted lexicographically.
//!
//! **Generated data — do not hand-edit.** Extracted programmatically (once) from
//! the C++ reference `host_core` source `src/bip39_english.cpp` (`kEnglishWords`)
//! during the Phase 03 M-T3.2 port, then frozen. Integrity is proven at test time
//! by [`super::tests`]: the count is 2048 and the SHA-256 of the newline-joined
//! list (with a trailing newline) is
//! `2f5eed53a4727b4bf8880d8f3f199efc90e58503646d9ff8eff3a2ed3b24dbda`, the
//! canonical BIP-39 English wordlist digest. The table is kept sorted so
//! [`super::word_index`] can binary-search it, exactly as the C++ `word_index`
//! did with `std::lower_bound`.

/// The 2048 BIP-39 English words, indexed by their canonical wordlist position
/// and sorted lexicographically (byte order). A `static` (not `const`) so the
/// 2048-entry table exists once in `.rodata` rather than being inlined per use.
pub(crate) static WORDS: [&str; super::WORD_COUNT] = [
