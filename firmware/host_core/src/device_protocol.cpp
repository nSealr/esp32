...
std::string handle_serial_frame(const std::string& line) {
    ...
    if (request.type == FrameType::Request &&
        request.payload_base64url == test_vectors::kPublicKeyRequestPayloadBase64Url) {
        return encode_serial_frame(SerialFrame{FrameType::Response, test_vectors::kPublicKeyResponsePayloadBase64Url});
    }
